...
struct Node
{
    int i;
    int j;
    int w1, w2;//коэффициенты эвристики 
    double g;
    double h;
    double f;
    const Node* parent;
    Node(int _i=-1, int _j=-1, double _g=1, double _h=0, Node* _parent=nullptr):i(_i), j(_j), g(_g), h(_h), f(_h+_g), parent(_parent){}
    
    /*void init_w1w2(std::string algorithm)
    {
        if (algorithm == "BFS")
        {
            w1 = 0;
